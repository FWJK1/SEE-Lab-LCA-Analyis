...
* Once you've built the environment, activate it with `conda activate SEE_BW` and then install the network visualization packages by entering `pip install -e .` in your terminal. 
...
* First, make sure that you have build your environment, installed the packages, and set up the SEE_LAB Project in Brightway as described in [Setup Steps](#steps). 
* Next, to run the interactive visualization, enter first `conda activate SEE_BW` and then `streamlit run lca_graph_viz/ui/app.py` into your terminal to run the visualization.
    * There are a number of command line options you can enter to customize how the visualizer runs. To run them, enter `streamlit run lca_graph_viz/ui/app.py -- ` and then one of the flags.
        * `--geomode` controls whether to put non-geolocated information in antartica or somewhere offshore. The antartica option is much faster and is the default.
        * `--subgraph_mode` controls whether to plot a random node or an lca. The default is an LCA.
        * `--lca_name` controls which lca you plot. The default is the placeholder Water Bottle LCA
        * `--lca_method` controls what method to use for the lca. Note that this must match up with the LCA that you use.
    * A full example:  ` streamlit run lca_graph_viz/ui/app.py -- --geomode antarctica --subgraph_mode lca`
    * If you have a case-study LCA saved in the brightway excel format, you can use that to guide your visualization by first loading it into brightway like how you loaded the "Water Bottle LCA" learning set in the `setup.ipynb` notebook, then entering it in brightway using the `--lca_name` and `--lca_method` flags. 
